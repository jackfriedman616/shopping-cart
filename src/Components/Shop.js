...
const Shop = ({isMobile, addToBasket, removeFromBasket, basket}) => {
  ...
  const hideComp = () => {
    ...
    setHiddenAccessories(isMobile ? true : hideAccessories)
  ...
  const hideAccessories = () => {
    ...
    setHiddenAccesories(!hiddenAccessories)
  }
        ...
        <ul className="sidebar">
          ...
          <div>
            ...
            <ul className={`${hiddenAccessories ? 'hidden' : ''}`}>
              ...
              <li className={`filter-item ${filterCurrent==='Stickers' ? 'filter-item-active' : ''}`} onClick={setFilter}>Stickers <i className="ms-s3 fa-regular fa-circle-xmark" onClick={setFilter}></i></li>
            </ul>
          </div>
          <div>
            <h5>Pre-builds</h5>
